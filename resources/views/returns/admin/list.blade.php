            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_category_table">
                ...
                <tbody class="fw-semibold text-gray-600">
                    ...
                    <tr>
                        ...
                        <td>
                            <small class="d-block">{{ $item->store->name }} / {{ $item->user->name }}</small>
                            <small>Talep Tarihi : <smal class="text-muted">{{ $item->created_at->format('d.m.Y H:i') }}</smal></small>
                        </td>
                        <td>{{ $item->adet }}</td>
                        <!--begin::Action=-->
                        <td class="text-end">
                            <a href="{{ route('returns.admin.detail', $item->id) }}" class="btn btn-sm btn-light-primary btn-active-light-info">İşlem</a>
                        </td>
                        <!--end::Action=-->
                    </tr>
                    @endforeach
